...
export class Ingresos {
  ...
  crear = async (datos) => {
    try {
      ...
      const {  id_tramite, monto, tipo, fecha_ingreso, detalle, usuario, created_at } = datos;
      ...
      const sql = `
        INSERT INTO ingresos (id,  id_tramite, numero, monto, tipo, fecha_ingreso, detalle, usuario, created_at, updated_at)
        VALUES (UUID(), ?, ?,?,?, ?, ?, ?, ?, ?)
      `;
  ...
  actualizar = async (id, datos) => {
    try {
      const { monto, tipo, fecha_ingreso, detalle, updated_at, usuario } = datos;
      const sql = `
        UPDATE ingresos 
        SET  monto = ?, tipo = ? , fecha_ingreso = ?, detalle = ?, usuario=?, updated_at = ?
        WHERE id = ? 
      `;
      // console.log(id, ' id_del modelo')
      const [result] = await pool.query(sql, [ monto, tipo, fecha_ingreso, detalle, usuario, updated_at, id]);
      return result.affectedRows > 0;
    } catch (error) {
